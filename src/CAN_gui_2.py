...
class CANSimulator(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.attack_mode:
                msg = self._generate_anomalous_message(self.attack_type)
            ...
            is_anom = self.detector.is_anomaly(msg)
            self.out_queue.put((msg, is_anom))
...
class CANGuiApp:
    def __init__(self, root):
        ...
        tk.Label(stats, text='Original:').pack(side='left')
        tk.Label(stats, textvariable=self.normal_var, width=6).pack(side='left')
        tk.Label(stats, text='Fake:').pack(side='left', padx=20)
        tk.Label(stats, textvariable=self.anom_var, width=6).pack(side='left')

        table_frame = tk.Frame(root)
        table_frame.pack(fill='both', expand=True, padx=8, pady=6)
        cols = ('Time', 'CAN ID', 'DLC', 'Data (hex)', 'Status')
        self.tree = ttk.Treeview(table_frame, columns=cols, show='headings', height=18)
        for c in cols:
            self.tree.heading(c, text=c)
            self.tree.column(c, width=150 if c != 'Data (hex)' else 450)
        ...
        footer = tk.Label(root, text='Green = Original. Red = Fake.', anchor='w')
        footer.pack(fill='x', padx=8, pady=(0, 8))

        self.msg_queue = queue.Queue()
        self.detector = AnomalyDetector()
        self.simulator = CANSimulator(self.msg_queue, self.detector)
    ...
    def toggle_attack(self):
        ...
        self.simulator.set_attack(self.attacking, self.attack_type_cb.get())
    ...
    def _process_queue(self):
        try:
            while True:
                msg, is_anom = self.msg_queue.get_nowait()
                self._add_message_to_table(msg, is_anom)
        except queue.Empty:
            pass
        self.root.after(100, self._process_queue)

    def _add_message_to_table(self, msg, is_anom):
        row = msg.to_row()
        status = 'FAKE' if is_anom else 'ORIGINAL'
        tag = 'anom' if is_anom else 'norm'
        self.tree.insert('', 0, values=(*row, status), tags=(tag,))
